...
exports.StaticDecoder = void 0;
exports.registerStaticDecoder = registerStaticDecoder;
exports.createStaticDecoder = createStaticDecoder;
/**
 * Registry des décodeurs statiques disponibles
 * Mappé par nom de contrat
 */
const staticDecoders = new Map();
/**
 * Registry des imports statiques pour éviter les imports dynamiques multiples
 */
const staticDecoderImports = new Map();
...
function loadStaticDecoder(contractName) {
    ...
    if (importFn) {
        try {
            ...
            throw new Error(`Failed to load registered static decoder for contract "${contractName}". ` +
    ...
    try {
        ...
        throw new Error(`Failed to load static decoder for contract "${contractName}". ` +
...
class StaticDecoder {
    ...
    decodeEvent(eventData, topics, contractAddress, blockHeight) {
        ...
        if (this.decoderModule.decodeEventWithRouting && contractAddress && blockHeight !== undefined) {
            const decoded = this.decoderModule.decodeEventWithRouting(signatureHex, eventData, topics, {
        ...
        throw new Error(`Static decoder for "${this.contractName}" does not export decodeEventWithRouting or decodeEvent. ` +
            `Available exports: ${Object.keys(this.decoderModule).join(', ')}`);
    }
}
exports.StaticDecoder = StaticDecoder;
/**
 * Factory pour créer un décodeur statique
 */
function createStaticDecoder(config) {
